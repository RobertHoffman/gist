...
start_date, end_date = '2019-01-01', '2021-11-23'
w.start()
...
# 读取指标库
inds = pd.read_excel(r'E:\桌面\建投工作文件\中观数据库\中观数据指标库.xlsx', sheet_name='指标列表', engine='openpyxl')
inds_str = ','.join(inds['Wind编号'].tolist())

# 读取已有数据、日期
if not os.path.exists(r'E:\桌面\建投工作文件\中观数据库\中观原始数据.csv'):
    exist_data = pd.DataFrame()
    exist_days = []
else:
    exist_data = pd.read_csv(r'E:\桌面\建投工作文件\中观数据库\中观原始数据.csv', index_col=0)
...
output_data.index = pd.DatetimeIndex(output_data.index)  # 将索引化作字符串
...
output_data.to_csv(r'E:\桌面\建投工作文件\中观数据库\中观原始数据.csv')

# 获取周频、月频指标列表
week_list = inds[inds['频率'] == '周']['Wind编号'].tolist()
month_list = inds['Wind编号'].tolist()
...
# 降采样
week_data = pd.concat([week_label, output_data[week_list].resample('W-Sun').last().fillna(method='ffill')], axis=0)
month_data = pd.concat([month_label, output_data.resample('M').last().fillna(method='ffill')], axis=0)
week_data.index = list(map(lambda x: x.strftime('%Y-%m-%d') if type(x) != str else x, week_data.index))
month_data.index = list(map(lambda x: x.strftime('%Y-%m-%d') if type(x) != str else x, month_data.index))

# 输出数据
with pd.ExcelWriter(r'E:\桌面\建投工作文件\中观数据库\中观数据底层表.xlsx', datetime_format='YYYY-MM-DD') as writer:
